...
export function registerPublishCandidate(_options: PublishCandidateOptions): string {
...
export function getPublishCandidate(_token: string): PublishCandidate | undefined {
  // cleanupExpiredCandidates();
  // return candidates.get(token);
  return undefined;
}

export function removePublishCandidate(_token: string): void {
  // candidates.delete(token);
}
...
export async function publishFileDirectlyToArena(
  _options: DirectArenaPublishOptions
): Promise<boolean> {
