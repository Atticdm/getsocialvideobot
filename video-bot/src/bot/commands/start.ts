...
⚠️ **Важно:** Используя бота, вы автоматически соглашаетесь с условиями использования (/terms). Переозвучка голоса является симуляцией и не имеет отношения к реальным людям. Пользователь несет полную ответственность за скачиваемый контент.`;
    ...
    if (!isBlockedError) {
      try {
        ...
        logger.error({ error: replyError, originalError: error, userId: ctx.from?.id }, 'Failed to send error message in start command');
      }
    } else {
      logger.warn({ userId: ctx.from?.id }, 'User blocked the bot, skipping error message in start command');
    }
  }
}
