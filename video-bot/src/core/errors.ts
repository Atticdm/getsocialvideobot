...
export function toUserMessage(error: AppError): string {
  const messages: Record<ErrorCode, string> = {
    [ERROR_CODES.ERR_PRIVATE_OR_RESTRICTED]: '❌ Video is private, age-restricted, or requires login. This can also be caused by geo-restrictions. For Instagram, try setting INSTAGRAM_COOKIES_B64. For YouTube, try setting YOUTUBE_COOKIES_B64. For geo-bypass, try setting GEO_BYPASS_COUNTRY environment variable.',
